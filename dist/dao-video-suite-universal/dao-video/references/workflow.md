...
- Route black-gold visual work to `$dao-blackgold-video`.
- Route manuscript, calligraphy, Remotion parallax, and water-reflection work to `$dao-ink-video`.
- Use scene prefixes `远景|`, `近景|`, or `围合|` with `ark_images.py` when the selected branch generates Ark scenes.
- Do not mix one branch's cover palette or visual rules into another branch's video prompts without an approved experiment.
